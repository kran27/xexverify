...
constexpr uint32_t MAX_BLOCK_SIZE = 0x10000;  // Maximum sane block size (64KB)
...
// Exhaustive brute force: try combinations in order of likelihood
//
// Key insight: If garbage clusters are INSERTED, the valid data remains CONTIGUOUS.
// So we check in this order:
// 1. Only final cluster wrong - try each cluster as final
// 2. Last N clusters form a contiguous group elsewhere
// 3. Full exhaustive search (non-contiguous)
// frag_offset: If non-zero, clusters before this offset are known valid
static bool bruteforce_block(FILE* file, int64_t block_offset, uint32_t block_size,
  ...
  // By default, first cluster is fixed (validated by previous block)
  ...
  std::unique_ptr<uint8_t[]> fixed_data_start = std::make_unique<uint8_t[]>(fixed_bytes_start);
  ...
  printf("Reading search data: 0x%llX bytes (%.2f MB) from 0x%llX to 0x%llX\n",
         static_cast<unsigned long long>(data_to_read), data_to_read / (1024.0 * 1024.0),
         static_cast<unsigned long long>(search_start), static_cast<unsigned long long>(search_end_offset));
  
  std::unique_ptr<uint8_t[]> search_data = std::make_unique<uint8_t[]>(data_to_read);
  ...
  std::unique_ptr<uint8_t[]> assembled = std::make_unique<uint8_t[]>(total_cluster_bytes);
  std::unique_ptr<uint8_t[]> block_buffer = std::make_unique<uint8_t[]>(block_size);
  std::unique_ptr<uint8_t[]> decrypt_buffer = std::make_unique<uint8_t[]>(block_size);
  ...
  auto test_combination = [&](const std::vector<uint32_t>& indices) -> bool {
    ...
    for (uint32_t i = 0; i < clusters_to_bruteforce; i++) {
      ...
      memcpy(assembled.get() + fixed_bytes_start + i * CLUSTER_SIZE,
...
// Find next block: Scan forward from current position looking for valid block headers
//
// When a block is unrecoverably fragmented, we can try to find where the NEXT block starts.
// 
// Key insight: XEX blocks are laid out CONTIGUOUSLY in the data stream. If block N starts
// at offset X with size S, block N+1 starts at offset X+S. Blocks are NOT cluster-aligned.
//
...
//   4. If decrypted size looks valid (0 or 0x1000-0x10000), it's a candidate
//
static void find_next_block(FILE* file, int64_t current_block_offset, uint32_t current_block_size,
...
int main(int argc, char* argv[]) {
  ...
  std::string frag_offset_str = result["f"].as<std::string>();
  std::string scan_end_str = result["E"].as<std::string>();
  ...
  if (err != 0 || !file) {
    char errbuf[256];
    if (strerror_s(errbuf, sizeof(errbuf), err)) {
  ...
  xex::XexDirectoryEntry* dir_entries = reinterpret_cast<xex::XexDirectoryEntry*>(headers.data() + sizeof(xex::XexHeader));
  ...
  xex_opt::XexFileDataDescriptor* data_desc = 
  ...
  if (format != xex_opt::XexDataFormat::Compressed && format != xex_opt::XexDataFormat::DeltaCompressed) {
    ...
    printf("For uncompressed files, you would need to verify via PE structure or image hash.\n");
    if (fclose(file))
    {
      printf("ERROR: fclose failed\n");
    }
  ...
  if (encrypted) {
    if (key_index < 0) {
      printf("File is encrypted but no key specified.\n");
      printf("Use -k 0 for retail key, -k 1 for devkit key.\n");
      printf("Proceeding without decryption - hashes will NOT match!\n\n");
      encrypted = false;  // Disable decryption
    } else {
      // Read security info to get encrypted session key
      if (security_info_offset) {
        xex2::SecurityInfo* sec_info = reinterpret_cast<xex2::SecurityInfo*>(headers.data() + security_info_offset);
  ...
  xex_opt::XexCompressedDataDescriptor* comp_desc = 
  ...
  while (current_desc.Size) {
    ...
    // Read block data
    std::unique_ptr<uint8_t[]> block_data = std::make_unique<uint8_t[]>(current_desc.Size);
    ...
    xex_opt::XexDataDescriptor* next_desc = reinterpret_cast<xex_opt::XexDataDescriptor*>(block_data.get());
